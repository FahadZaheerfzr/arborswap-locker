    ...
    return (
        <div className='p-9 w-[90%] max-w-[520px] bg-white rounded-[10px]'>
            ...
            <div className="mt-14 grid grid-cols-2 md:grid-cols-3 gap-8"
                ...
                {wallets.map((wallet) => (
                    <button className="flex focus:outline outline-primary-green py-4 px-4 flex-col items-center justify-center" key={wallet.id}>
                ...
                {wallets.map((wallet) => (
                    <button className="flex flex-col items-center focus:outline outline-primary-green py-4 px-4 justify-center cursor-pointer" key={wallet.id}
                        onDoubleClick={handleConnection}>
                        <img src={wallet.image} alt={wallet.name} className="w-[50px] h-[50px] mb-4" />
                        <span className="font-gilroy font-semibold text-dark-text dark:text-light-text">{wallet.name}</span>
                    </button>
