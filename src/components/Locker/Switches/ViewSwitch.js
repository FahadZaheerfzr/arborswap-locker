...
export default function ViewSwitch({ cardFormat, setCardFormat }) {
    return (
        <div className="flex bg-[#F5F1EB] dark:bg-dark-1">
            <div className={`absolute bg-white dark:bg-dark-1 w-11 h-11  rounded-md flex justify-center items-center ease-in-out duration-300 ${cardFormat === 'grid' ? 'translate-x-[44px]' : ''}`}>
                {cardFormat === 'list' ?
