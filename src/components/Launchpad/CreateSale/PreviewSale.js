...
export default function PreviewSale({ token, setActive, saleObject, saleType }) {
  ...
  return (
    <div className="">
      ...
      {saleType === 'fairlaunch' && <PreviewDetails name={'Amount to be sold'} value={'100,000,000 SXP'} />}

      {saleType === 'standard' && (
        <PreviewDetails
          name={'Presale Rate'}
          value={saleObject.presalePrice + ' ' + token.symbol + ' = 1 ' + saleObject.currency.symbol}
        />
      )}
      ...
      {saleType !== 'fairlaunch' && (
        <div>
          ...
          <PreviewDetails
            ...
            value={saleObject.minAllocation.toLocaleString() + ' ' + saleObject.currency.symbol}
          />
          <PreviewDetails
            name={'Maximum Allocation'}
            value={saleObject.maxAllocation.toLocaleString() + ' ' + saleObject.currency.symbol}
          />
          <PreviewDetails name={'Amount to be sold'} value={'100,000,000 SXP'} />
        </div>
      )}
      {saleType === 'fairlaunch' && <PreviewDetails name={'Sale Type'} value={'Fairlaunch'} />}
      {saleType === 'standard' && (
        <PreviewDetails
          name={'Sale Type'}
          value={saleObject.whiteisting ? 'Whitelist Enabled' : 'Whitelist Disabled'}
        />
      )}
      {saleType === 'private' && (
        <PreviewDetails
          name={'Sale Type'}
          value={saleObject.whiteisting ? 'Private Sale, Whitelist Enabled' : 'Private Sale, Whitelist Disabled'}
        />
      )}
      ...
      {saleType === 'standard' && (
        <PreviewDetails
          name={'Listing rate'}
          value={saleObject.listing + ' ' + token.symbol + ' = 1 ' + saleObject.currency.symbol}
        />
      )}
